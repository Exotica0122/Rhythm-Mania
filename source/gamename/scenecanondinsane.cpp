...
SceneCanonDInsane::SceneCanonDInsane()
	: m_pCursor(0)
...
bool
SceneCanonDInsane::Initialise(Renderer& renderer, FMOD::System* soundSystem)
{
	renderer.CreateStaticText("J. Pachelbel - Canon (Cranky Remix) (lcfc) [Collab Insane]", 25);
	m_pDifficultyText = renderer.CreateSprite("J. Pachelbel - Canon (Cranky Remix) (lcfc) [Collab Insane]");
	...
	m_pSongClass->Initialise(renderer, soundSystem, "songs\\847812 J Pachelbel - Canon (Cranky Remix)\\J. Pachelbel - Canon (Cranky Remix) (lcfc) [Collab Insane].osu");
...
void
SceneCanonDInsane::Process(float deltaTime, InputSystem& inputSystem)
{
	// Implement UI stuffs
...
bool
SceneCanonDInsane::GetIsPaused()
{
	return m_bIsSongPause;
}

void
SceneCanonDInsane::DebugDraw()
{
...
void
SceneCanonDInsane::Draw(Renderer& renderer)
{

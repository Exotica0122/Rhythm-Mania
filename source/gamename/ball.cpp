...
bool
Ball::Initialise(Renderer& renderer, bool isGood)
{
	m_pSprite = renderer.CreateSprite("sprites\\ball.png");
	RandomiseSize();
	const float MAX_SPEED = 250.0f;
	...
	{
		m_position.x = static_cast<float>(GetRandom(EDGE_LIMIT, SCREEN_WIDTH - EDGE_LIMIT));
		m_position.y = static_cast<float>(EDGE_LIMIT);
	}
	else
	{
		m_position.x = static_cast<float>(GetRandom(EDGE_LIMIT, SCREEN_WIDTH - EDGE_LIMIT));
		m_position.y = static_cast<float>(SCREEN_HEIGHT);
	}
...
void
Ball::FollowMouse(InputSystem& inputSystem)
{
...
Ball::Shrink()
{
	float prevScale = m_pSprite->GetScale();
	float newScale = prevScale -= 0.1f;
	m_pSprite->SetScale(newScale);
}
